"""
Gate 6 — Multi-LoRA routing works correctly under expert parallelism (EP=8).
...
post-dispatch, and that per-adapter gradient isolation holds under EP=8.

With EP=8 and Qwen3.5-35B-A3B (256 experts), each rank holds 32 local experts.
This exercises the full AlltoAll at natural scale — every sequence's 8 active
experts are likely distributed across all 8 EP ranks.

Run with:
    torchrun --nproc-per-node=8 -m pytest tests/multi_lora/test_ep8.py -v -s
"""
import torch
...
@requires_cuda
def test_ep8_preallocate_and_registry():
    """preallocate_adapters and registry methods work correctly under EP=8."""
...
@requires_cuda
def test_ep8_routing_changes_output():
    """Non-zero LoRA adapter changes model output compared to base-only, under EP=8."""
    init_megatron(ep=EP_SIZE)
    _patcher.apply_patch()
...
@requires_cuda
def test_ep8_grad_isolation():
    """Under EP=8, adapter 2 receives no gradient when batch routes only to adapter 1."""
    init_megatron(ep=EP_SIZE)
    _patcher.apply_patch()
...
@requires_cuda
def test_ep8_determinism():
    """Same input + same routing is bit-identical across two calls, under EP=8."""
    init_megatron(ep=EP_SIZE)
    _patcher.apply_patch()
...
@requires_cuda
def test_ep8_expert_dispatch_indices():
    """RouterReplay sets _expert_lora_indices on the root model during expert compute.

    With EP=8 and 256 experts / 8 active per token, tokens are dispatched across
    all 8 ranks. Checks that _expert_lora_indices is present and in the valid range.
    """
    init_megatron(ep=EP_SIZE)
    _patcher.apply_patch()
    ...
    if rank() == 0:
        print(f"\n[EP=8] _expert_lora_indices range: [{observed['min_idx']}, {observed['max_idx']}]")


@requires_cuda
def test_ep8_loss_decreases():
    """Under EP=8, training loss decreases after optimizer steps with per-task routing."""
    init_megatron(ep=EP_SIZE)
    _patcher.apply_patch()
    ...
    if rank() == 0:
        print(f"\n[EP=8] early_avg={early_avg:.4f} late_avg={late_avg:.4f} drop={drop:.1%}")
