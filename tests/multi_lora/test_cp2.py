"""
Context Parallelism (CP=2) — multi-LoRA per-sequence routing tests.

Verifies that per-sequence LoRA routing is correct when the sequence
dimension is sharded across 2 context-parallel ranks. CP preserves the
batch dimension, so batch-slot→adapter mapping should be unaffected.

Run with:
    torchrun --nproc-per-node=2 -m pytest tests/multi_lora/test_cp2.py -v -s
"""
import torch
...
@requires_cuda
def test_cp2_preallocate_and_registry():
    """preallocate_adapters and registry work correctly under CP=2."""
    _init_megatron()
...
@requires_cuda
def test_cp2_routing_changes_output():
    """Non-zero LoRA adapter changes model output vs base-only under CP=2."""
    _init_megatron()
    _patcher.apply_patch()
...
@requires_cuda
def test_cp2_grad_isolation():
    """Under CP=2, adapter 2 receives no gradient when batch routes only to adapter 1."""
    _init_megatron()
    _patcher.apply_patch()
...
@requires_cuda
def test_cp2_determinism():
    """Same input + same routing is bit-identical across two calls under CP=2."""
    _init_megatron()
    _patcher.apply_patch()
...
@requires_cuda
def test_cp2_loss_decreases():
    """Under CP=2, training loss decreases with per-task LoRA routing (Gate CP)."""
    _init_megatron()
    _patcher.apply_patch()
    ...
    # seq_len must be divisible by CP_SIZE
    ...
    if _rank() == 0:
        print(f"\n[CP=2] early_avg={early_avg:.4f} late_avg={late_avg:.4f} drop={drop:.1%}")
